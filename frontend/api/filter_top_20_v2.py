...
# Redundant ISS modules to exclude from our 15 satellites
EXCLUSION_KEYWORDS = ['POISK', 'NAUKA', 'CREW DRAGON', 'PROGRESS', 'CYGNUS', 'SOYUZ-MS', 'PIRS', 'RASSVET']
...
for obj in objects:
    ...
    is_excluded = any(keyword in obj.name.upper() for keyword in EXCLUSION_KEYWORDS)
    if is_excluded:
        continue
...
for s in valid_sats:
    ...
    if "DEB" in obj.name.upper() or "R/B" in obj.name.upper() or "DEBRIS" in obj.name.upper():
        debris.append((c, obj))
    else:
...
# Delete everything not in our top 20
db.query(TrackedObject).filter(TrackedObject.norad_id.not_in(keep_ids)).delete(synchronize_session=False)

# Delete old conjunctions
db.query(Conjunction).delete()

db.commit()

print(f"Kept 5 debris and 15 satellites with highest conjunction potential (Excluding docked ISS modules).")
db.close()
