...
- **Interactive 3D Globe:** Visualizes Earth, day/night cycles, and live satellite positions using `react-globe.gl` and Three.js.
...
- `react-plotly.js` (Data visualization)
...
2. **Backend:** The `api/` directory contains `index.py`, which Vercel's Python runtime natively converts into Serverless Functions. A `vercel.json` file ensures all `/api/*` traffic routes into the FastAPI app.
...
```bash
curl -X POST https://orbital-guard-one.vercel.app/api/catalog/sync
```
